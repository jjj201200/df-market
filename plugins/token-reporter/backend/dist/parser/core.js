...
import { parseSlashCommandContent, parseBashInputContent, parseBashOutputContent, parseLocalCommandStdout, isSlashCommandWrapperContent, } from './content.js';
...
export async function parseSession(filePath) {
            ...
            if (bashOutput) {
                if (lastCommandEvent && !lastCommandEvent.output) {
                    ...
                    if (bashOutput.stdout)
                        parts.push(bashOutput.stdout);
                    if (bashOutput.stderr)
                        parts.push(bashOutput.stderr);
    ...
    for (const rec of lines) {
        ...
        if (cc.ephemeral_1h_input_tokens)
            cacheTtl.ephemeral1h += cc.ephemeral_1h_input_tokens;
        if (cc.ephemeral_5m_input_tokens)
            cacheTtl.ephemeral5m += cc.ephemeral_5m_input_tokens;
    }
