export function parseSlashCommandContent(content) {
...
export function parseLocalCommandStdout(content) {
    if (typeof content !== 'string')
        return null;
    const m = content.match(/<local-command-stdout>([\s\S]*?)<\/local-command-stdout>/);
    return m ? m[1] : null;
}
export function isLocalCommandCaveat(content) {
    return typeof content === 'string' && /<local-command-caveat>/.test(content);
}
export function parseBashInputContent(content) {
    if (typeof content !== 'string')
        return null;
    const m = content.match(/<bash-input>([\s\S]*?)<\/bash-input>/);
    return m ? m[1] : null;
}
export function parseBashOutputContent(content) {
    if (typeof content !== 'string')
        return null;
...
export function isSlashCommandWrapperContent(content) {
    if (typeof content !== 'string')
        return false;
    return (parseSlashCommandContent(content) !== null ||
