...
export async function parseSubagentFile(filePath: string): Promise<{
  totalTurns: number;
...
export async function collectSubagentStats(sessionDir: string): Promise<Record<string, SubagentStats>> {
  const subagentsDir = path.join(sessionDir, 'subagents');
  if (!fs.existsSync(subagentsDir)) {
    return {};
  }

  const subagents = new Map<string, SubagentStats>();
