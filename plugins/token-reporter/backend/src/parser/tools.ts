export function toolNameToCls(name: string): string {
...
export function parseMcpToolName(name: string): { server: string; method: string } | null {
...
export function buildInputArgs(toolName: string, input: Record<string, unknown>): { k: string; v: string; vc: string }[] {
  const entries: { k: string; v: string; vc: string }[] = [];
...
export function buildParamsSummary(toolName: string, input: Record<string, unknown>): string {
  const n = (toolName || '').toLowerCase();
  if (n === 'bash') return typeof input.command === 'string' ? input.command : '';
  if (n === 'read') {
  ...
  if (n === 'edit' || n === 'write') return typeof input.file_path === 'string' ? input.file_path : '';
  if (n === 'grep') {
    const pattern = typeof input.pattern === 'string' ? input.pattern : '';
    const glob = typeof input.glob === 'string' ? input.glob : '';
    return `pattern: "${pattern}"${glob ? ' · ' + glob : ''}`;
  }
  if (n === 'glob') return typeof input.pattern === 'string' ? input.pattern : '';
  const first = Object.values(input).find((v) => typeof v === 'string');
  return (first as string) || JSON.stringify(input).slice(0, 80);
}
