# skill-audit 核查维度手册（8 维度）

本 reference 详述 `skill-audit` 的 8 个核查维度。主流程执行阶段 Read 本文件，按每个维度的"检查内容 / 方法 / 典型漂移"逐项做。
...
**检查**：SKILL.md 正文每一段是否承担明确的控制信号职责；是否存在"去掉后 Claude 执行不会出错"的稀释段落

**方法**：

1. Read SKILL.md 全文，按段落切分
2. 对每段回答：**"删掉这段，Claude 执行本 skill 的判断会不会变化"**
3. 答案为"不会变化"但该段仍留着 → 候选稀释剂
4. 报告时必须附被识别段落的**原文首末行引用**（`file:line` + 首行文本），不允许只写"某节过长"这类模糊描述

**典型稀释剂模式**（识别参考，不穷举）：

- **辩护性叙述**："这很重要，因为……" 当"为什么"已在前文说清时
- **跨文件重复**：本 skill 讲过的规则在 reference 里又完整讲一遍（应保留指针，删重复正文）
- **示例堆砌**：3 个同类示例只保留 1 个即可
- **装饰性总结**：章末"综上所述 / 总之"重复上文要点

**不算稀释**（即便信号重复也要保留）：

- **反模式警示**（`❌ 不要……`）——负面信号对模型更强
- **硬规则明确断言**（"必须 / 禁止 / 不允许 / 硬性"）——即便简短也要留
- **承接句**（"以下按 A / B / C 展开"）——帮助结构定位
