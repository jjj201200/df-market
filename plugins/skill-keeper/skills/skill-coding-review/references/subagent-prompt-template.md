...
当发现以下类别的问题时，**必须主动 Grep 全仓**确认其它同类位置是否也该一起处理——不允许"只报本轮 diff 里的一处"：

- **可抽的字面量**：发现某字符串/数字出现 ≥ 3 次可抽常量 → Grep 全仓该字面量，把所有出现点列入结论
- **可复用的函数**：发现本轮 diff 有重复实现 → Grep 全仓有无同名或同形函数已存在
- **错误处理模式**：发现一处 `try/catch` 吞错 / 静默 `return null` → Grep 全仓同类 pattern，列出所有同类位置
- **接口签名 / 类型声明变更**：Grep 全仓所有调用点（不只 diff 里的）
- **持久层 / 路由 / 全局配置**：凡涉及这些就默认全仓扫

报告须在结论里显式标注"已扫关联路径 N 处，其中 M 处为同类"——有数字，不含糊。
...
### 关联路径扫描
- 本轮涉及的扩展类别：<字面量 / 函数 / 错误处理 / 签名 / 持久层等>
- 已扫关联路径 N 处，其中 M 处为同类（列出 M 处的 file:line）
- 如不涉及扩展类别：显式写"本轮 diff 未触发关联路径扩展场景"
