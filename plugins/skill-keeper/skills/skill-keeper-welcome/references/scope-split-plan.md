...
AUQ 单个问题最多 4 个 options，而 skill-keeper 的派生候选有 5 个，必须拆分。本文件固定下来拆分方案，welcome 环节 5.1 **直接 Read 本文件后按结构填 AUQ**，不要临时设计新拆法。

---

## 方案（同轮发起 2 个 multiSelect 问题）

**问题 A（multiSelect = true）**：勾选要派生的「编排 + 守门」三件套

```json
{
  "question": "勾选要派生的「编排 + 守门」三件套（可多选）",
  "header": "编排/守门",
  "multiSelect": true,
  "options": [
    {
...
| 拆法                             | 理由                                                                  |
| -------------------------------- | --------------------------------------------------------------------- |
| 三件套 A + 两件套 B              | 按**使用频率**分组：A 是高频的回顾+守门闭环，B 是低频的周期性审计     |
