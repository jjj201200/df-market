...
description: "SKILL.md 落盘前增量一致性快检：修改 / 新增 / 删除 / 重命名 SKILL.md 前核查触发词覆盖、description 与正文同步、交叉引用可达、职责边界，漂移则阻断落盘。触发词：skill 落盘快检、skill-sync-check、skill 落盘守门。"
---

# Skill 落盘前增量一致性快检（通用方法论）

在 SKILL.md 变更写入磁盘之前，快速核对本次变更引发的级联影响。只管**本次变更涉及的 skill 及其级联**，不做全量扫描。
